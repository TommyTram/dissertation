...
To demonstrate the effect of accounting for uncertainty, the results include two types of experiments. In the first type, the trained agent is evaluated on scenarios within the training set. In the second type, the agent is evaluated on scenarios outside the training set.
...
This section shows the results of ensemble RPF method compared to the DQN method used in \paperLSTM, but without the \gls{lstm} layer.
The ensemble RPF method outperforms the DQN method, both in terms of training speed and final performance, when the resulting agents are tested on scenarios that are similar to the training scenarios. When the fully trained ensemble RPF agent is exposed to situations that are outside the training distribution, the agent indicates a high uncertainty and chooses safe actions, whereas the DQN agent collides with other vehicles.
...
\begin{figure}[h]
	...
	\label{fig:RPFreturnAndSuccess}
\end{figure}

The average return and the average proportion of episodes where the ego vehicle reached the goal, as a function of number of training steps, is shown in Figure~\ref{fig:RPFreturnAndSuccess}, for the test episodes. The figure also shows the standard deviation for $5$ random seeds, which generates different sets of initial parameters of the networks and different training episodes, whereas the test episodes are kept fixed. The results show that the ensemble RPF method both learns faster, yields a higher return, and causes less collisions than the \gls{dqn} method. %A few collisions still remain, but how they can be mitigated is shown below.
%
...
As shown in Figure~\ref{fig:RPFreturnAndSuccess}, occasional collisions still occur during the test episodes when deploying the fully trained ensemble RPF agent. 
In one particular example of a collision, using the zones from Figure~\ref{fig:zones}, the agent fails to brake enough in zone 3 and take an incorrect action in zone 2, which leads to a collision in zone 1. 
However, Figure~\ref{fig:RPFcvDuringCrash} show that the estimated uncertainty increases significantly during the time before the collision (zone 2), when the incorrect actions are taken. 
When applying the confidence criterion, the agent, aware of high uncertainty, brakes early enough in zone 2, thus avoiding collisions. The confidence criterion was applied to all test episodes, effectively eliminating all collisions for scenarios within the training set.
...
To evaluate the agents' ability to detect unseen situations, a fully trained ensemble RPF agent was tested in scenarios outside the training distribution. The same testing scenarios as in the previous section were used, with the exception that the speed of the surrounding vehicles was set to a single deterministic value, which was varied during different runs in the range $v_n=[10,20]$ m/s. 
...
The ensemble RPF agent avoided collisions by not taking a bad action in zone 2, by accounting for the uncertainty of the Q-value estimate. However, is it possible reduce the number of collisions by accounting for the uncertainty of the input when in zone 3? 
In \paperBelief, the uncertainty about the intention state $\zeta_n$ is represented by a belief state, which is a probability distribution over possible true states. A particle-filter was used to give the probability distribution, henceforth referred to as intention distributions. 

Using the intention distributions, two methods were proposed: QMDP-IE and QID. 
QID trained the intention distributions as the intention state $\zeta_n$, while QMDP-IE is derived from the QMDP algorithm~\cite{Littman1995}. By assuming that the intention state $\zeta_n$ can be derived by looking at the history after crossing the intersection, allows the QMDP algorithm to combine elements of both \gls{pomdp}s and \gls{mdp}s. In QMDP-IE and QMDP the \gls{nn} is trained using true intention state and the QMDP calculates the Q-value for each belief state (potential true state). QMDP then computes the expected Q-value for each action by taking the weighted average of the Q-values, while QMDP-IE chooses one intention state using a threshold $\zeta_\mathrm{threshold}$ on the intention distribution.
...
\begin{figure}[h]
	\centering
			...
			\vspace{-0.8cm}
			\caption{QMDP-IE success time for different $\zeta_\text{threshold}$ values. Increasing $\zeta_\text{threshold}$ lowers the collision rate while at the same time increasing the safe stop rate and the time it takes to reach a success state.}
	\label{fig:thesis_intent_threshold}
\end{figure}

QMDP-IE, in particular, offered adjustable aggressiveness post-training, yielding a collision rate close to the Oracle DQN with an optimal threshold value $\zeta_\mathrm{threshold}$.
Furthermore, QID policies collision rate was comparable to QMDP-IE for scenarios outside the training set. 

\subsection{Results within the training set}
The results from the experiments show that QMDP-IE outperformed QID. 
...
Starting with QMDP-IE, we first demonstrate how the $\zeta_\text{threshold}$ is chosen and then compare its results with QMDP. Both algorithms use the same network weights as the Oracle DQN, but QMPD-IE is less computational than QMDP since the threshold $\zeta_\text{threshold}$ transforms the set of particles into a point estimate of the intention state. Therefore, the choice of $\zeta_\text{threshold}$ is an important hyperparameter that directly correlates with the aggressiveness of the agent.
...
Looking at \ref{tab:belief_results_summary}, QPF has the highest collision rate at \SI{12.15}{\percent} and the second lowest success time out of all considered algorithms, making it the most aggressive policy out of all four algorithms. 
...
In the overtake scenario, both QID and QPF have relatively low collision rates at \SI{2.53}{\percent} and \SI{3.7}{\percent} respectively, which is lower when compared to the trained scenarios in \ref{tab:belief_results_summary}. 
