...
The previous chapter formulated the \gls{pomdp} and introduced how deep Q-learning methods can be used to create decision-making agents capable of navigating through intersections. An important advantage of \gls{rl} methods is their ability to scale to different types of scenarios simply by training on them. However, a drawback of deep Q-learning methods is the usage of neural networks \gls{nn}, which provide a black-box solution that does not indicate any confidence value for its decisions any hidden states it may estimate.

The motivation for handling uncertainty is critical. In this chapter, we present two approaches to managing uncertainty: one addresses the uncertainty in the output of the DQN, and the other addresses the uncertainty in the intention estimation that is fed as an input to the DQN.

The previous chapter formulated \gls{pomdp} and introduced how deep Q-learning methods can be used to create decision-making agents capable of navigating through intersections. 
An important advantage of \gls{rl} methods is the ability to scale to different types of scenarios simply by training on them. However, a drawback of deep Q-learning methods is the usage of a \gls{nn} which provide a black-box solution that does not indicate any confidence value for its decisions any hidden states it may estimate.
% As discussed in Chapter~\ref{ch:introduction}, an important advantage of learning-based approaches, compared to manually specified systems, is that they could scale to all types of driving situations. 

The motivation of handling uncertainty. In this chapter we present two approaches to handling the uncertainty, \paperEnsamblen addresses the uncertainty of output from the \gls{dqn}, and \paperBelief addresses the uncertainty of the input to the \gls{dqn}, more specifically the estimation of the intention state $\zeta_n$.
