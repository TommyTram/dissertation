...
% Latex complains if you use \\ here, so instead we use
% obeylines to get same formatting but remove the useless latex
% warnings
\bgroup\obeylines
\textbf{\ThePrintingLocationPageTitle}
\ifdefined \ThePrintingLocationPageSubitle \if \ThePrintingLocationPageSubitle\empty\else {\footnotesize\textit{\ThePrintingLocationPageSubitle}}
\fi\fi
~
...
~
~
\copyright~\TheYear~\textsc{\ThePrintingLocationPageAuthor}
All rights reserved.
~
~
...
Email: \verb|tommy.tram@chalmers.se|; \verb|tommy.tram@gmail.com|
\egroup
% end usage of obeylines
\vfill
...
\ifdefined \TheCoverImageCaption\ifdefined\TheCoverImage\if\TheCoverImage\empty\else\if\TheCoverImageCaption\empty\else
    Cover:
    \TheCoverImageCaption
    ~
\fi\fi\fi\fi
Printed by \ThePrinter
...
\ifdefined\TheDedication\if\TheDedication\empty\else
    ...
    % \centerline{{\small \emph{}}}
%    \vfill
%    \begin{center}
%        \includegraphics[width=1cm]{TemplateFiles/cover/swecro.png}
%    \end{center}
